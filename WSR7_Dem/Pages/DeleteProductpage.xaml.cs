...
namespace WSR7_Dem.Pages
{
    ...
    public partial class DeleteProductpage : Page
    {
        ...
        private void DeleteButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                WSR7EntitiesDemexam.GetContext().Product.Remove((sender as Button).DataContext as Product);
                WSR7EntitiesDemexam.GetContext().SaveChanges();
                MessageBox.Show("Выполнено", "Продукт был успешно удален", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch
            {
                MessageBox.Show("Ошибка", "Не удалось удалить продукт", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
    }
